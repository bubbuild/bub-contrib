...
def test_schedule_trigger_preserves_interval_baseline(scheduler, context):
    ...
    if job and job.next_run_time:
        ...
        time_diff = abs(
            (job.next_run_time - expected_next_after_original).total_seconds()
        )
        assert time_diff < 1, (
            f"Next run shifted from original baseline: {job.next_run_time} vs expected {expected_next_after_original}"
        )
