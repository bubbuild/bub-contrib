...
from bub_ag_ui.channel import AGUIChannel, _select_message_payload
from bub_ag_ui.config import load_settings
...
class AGUIPlugin:
    ...
    async def save_state(
        ...
        if self._channel.input_for(message) is None:
            return
        await self._channel.publish_result(
            message, state=state, model_output=model_output
        )

    @hookimpl
    async def on_error(
        self, stage: str, error: Exception, message: Envelope | None
    ) -> None:
        del stage
...
def _public_state_from_input_state(value: Any) -> TurnState:
...
def _transport_state_from_input(input_data: Any) -> dict[str, Any]:
    return {
        ...
        "forwarded_props": _serialize_value(
            getattr(input_data, "forwarded_props", {}) or {}
        ),
    }
