...
_INPUT_ATTR = "_bub_ag_ui_input"
_RUN_ID_ATTR = "_bub_ag_ui_run_id"
...
class _ActiveRequest:
    ...
    assistant_message_id: str = field(
        default_factory=lambda: f"assistant-{uuid4().hex}"
    )
...
class AGUIChannel(Channel):
    ...
    def __init__(
        self, on_receive: MessageHandler | None, *, settings: AGUISettings | None = None
    ) -> None:
    ...
    def bind_receiver(self, on_receive: MessageHandler) -> None:
        self._on_receive = on_receive
    ...
    async def start(self, stop_event: asyncio.Event) -> None:
        ...
        self._server_task = asyncio.create_task(
            self._server.serve(), name="bub-ag-ui.server"
        )
    ...
    async def send(self, message: ChannelMessage) -> None:
        ...
        if message.kind == "error":
            await self.publish_error(
                message, error=content or "dispatch error", code="dispatch_error"
            )
            return
        ...
        if content and not active.assistant_started:
            ...
            not content and active.pending_model_output and not active.assistant_started
        ):
        ...
        await active.emit(
            RunFinishedEvent(
                ...
                result={"text": "".join(active.final_text_parts)}
                if active.final_text_parts
                else None,
            )
        )
        await active.close()
        await self._remove_request(
            active.input_data.thread_id, active.input_data.run_id
        )

    async def publish_error(
        self, message: ChannelMessage, *, error: str, code: str
    ) -> None:
        active = await self._lookup_request(message)
        if active is None or active.finished:
            return
        ...
        if (
            active.pending_snapshot
            and active.pending_snapshot != active.initial_snapshot
        ):
            await active.emit(
                StateSnapshotEvent(
                    type=EventType.STATE_SNAPSHOT, snapshot=active.pending_snapshot
                )
            )
        await self._emit_text_end(active)
        await active.emit(
    ...
    async def publish_state(
        self, message: ChannelMessage, state: dict[str, Any]
    ) -> None:
    ...
    async def register_request(
        self, input_data: RunAgentInput, *, encoder: EventEncoder
    ) -> _ActiveRequest:
        ...
        async with self._lock:
            ...
            self._active_runs_by_session.setdefault(input_data.thread_id, set()).add(
                input_data.run_id
            )
        return active
    ...
    async def _emit_stream_side_effect(
        self, active: _ActiveRequest, event: StreamEvent
    ) -> None:
        if active.finished:
            return
        if event.kind == "text":
        ...
        if event.kind == "tool_call":
            for index, call in enumerate(_event_items(event.data, "tool_calls")):
                tool_call_id = _tool_call_id(
                    call, fallback_prefix=active.input_data.run_id, index=index
                )
                ...
                await active.emit(
                    ToolCallStartEvent(
                        ...
                        parent_message_id=active.assistant_message_id
                        if active.assistant_started
                        else None,
                    )
                )
        ...
        if event.kind == "tool_result":
            for index, result in enumerate(_event_items(event.data, "tool_results")):
                tool_call_id = active.tool_call_ids.get(
                    index, f"{active.input_data.run_id}-tool-{index}"
                )
                await active.emit(
                    ToolCallResultEvent(
    ...
    async def _emit_initial_snapshot(self, active: _ActiveRequest) -> None:
        ...
        await active.emit(
            StateSnapshotEvent(
                type=EventType.STATE_SNAPSHOT, snapshot=active.initial_snapshot
            )
        )
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="Bub AG-UI channel")
        ...
        async def ag_ui_endpoint(input_data: RunAgentInput, request: Request):
            ...
            try:
                if self._on_receive is None:
                    raise RuntimeError("AG-UI channel is not bound to the Bub gateway")
...
def _select_message_payload(messages: list[Any]) -> tuple[str, list[MediaItem]]:
    ...
    source = (
        user_messages[-1] if user_messages else (messages[-1] if messages else None)
    )
    if source is None:
        return "", []
...
def _event_items(data: dict[str, Any], key: str) -> list[Any]:
    value = data.get(key)
