...
async def _cleanup(client: redis.Redis, prefix: str) -> None:
...
async def test_list_tapes_returns_sorted_names(store: RedisTapeStore) -> None:
    ...
    await store.append("alpha", TapeEntry.message({"role": "assistant", "content": "world"}))

    assert await store.list_tapes() == ["alpha", "beta"]


@pytest.mark.asyncio
async def test_tape_local_keys_share_one_cluster_hash_slot(store: RedisTapeStore) -> None:
    keys = [
...
async def test_tape_names_with_braces_do_not_collide(store: RedisTapeStore) -> None:
    ...
    await store.append(brace_tape, TapeEntry.message({"role": "user", "content": "brace"}))
    await store.append(plain_tape, TapeEntry.message({"role": "user", "content": "plain"}))
...
@pytest.mark.asyncio
async def test_append_assigns_incrementing_ids_and_reset_restarts(store: RedisTapeStore) -> None:
    await store.append("session", TapeEntry.message({"role": "user", "content": "first"}))
    await store.append("session", TapeEntry.message({"role": "assistant", "content": "second"}))
    ...
    await store.append("session", TapeEntry.message({"role": "user", "content": "third"}))
...
async def test_fetch_all_matches_republic_query_contract(store: RedisTapeStore) -> None:
    ...
    entries = list(await TapeQuery(tape=tape, store=store).between_anchors("a1", "a2").kinds("message").limit(1).all())
...
async def test_query_between_dates_is_inclusive(store: RedisTapeStore) -> None:
    ...
    entries = list(await TapeQuery(tape=tape, store=store).between_dates(date(2026, 3, 2), "2026-03-03").all())
    assert [entry.payload["content"] for entry in entries] == ["during"]


@pytest.mark.asyncio
async def test_query_combines_anchor_date_and_text_filters(store: RedisTapeStore) -> None:
    tape = "combined"
...
@pytest.mark.asyncio
async def test_duplicate_anchor_names_use_latest_start_and_first_following_end(store: RedisTapeStore) -> None:
    ...
    after_start = list(await TapeQuery(tape=tape, store=store).after_anchor("start").all())
    assert [entry.payload["content"] for entry in after_start if entry.kind == "message"] == ["kept", "after"]

    between = list(await TapeQuery(tape=tape, store=store).between_anchors("start", "end").all())
...
async def test_empty_string_anchor_is_indexed(store: RedisTapeStore) -> None:
    ...
    await store.append(tape, TapeEntry.message({"role": "user", "content": "after empty"}))
