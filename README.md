...
| [`packages/bub-dynamic-workflows`](./packages/bub-dynamic-workflows/README.md)       | `dynamic-workflows`    | Provides bee-on-tape dynamic workflow tools and CLI commands backed by Bub subagents, workspace task state, and tape anchors.                         |
| [`packages/bub-extism`](./packages/bub-extism/README.md)                             | `extism`               | Bridges selected Bub hooks to Extism WebAssembly plugins so extensions can be written in any Extism PDK language.                                    |
...
| [`packages/bub-kimi`](./packages/bub-kimi/README.md)                                 | `kimi`                 | Provides a `run_model` hook backed by the Kimi CLI, including persisted session resume support and temporary Bub skill wiring.                       |
| [`packages/bub-mcp`](./packages/bub-mcp/README.md)                                   | `mcp`                  | Exposes configured MCP servers as Bub tools, with `bub mcp` CLI commands to list, add, and remove server configs.                                    |
| [`packages/bub-qq`](./packages/bub-qq/README.md)                                     | `qq`                   | Provides a QQ Open Platform channel adapter for Bub message IO.                                                                                      |
| [`packages/bub-searxng-search`](./packages/bub-searxng-search/README.md)             | `searxng-search`       | Provides a `searxng.search` tool backed by a self-hosted SearXNG instance. Registers the tool only when `BUB_SEARXNG_SEARCH_BASE_URL` is configured. |
...
| [`packages/tape-dataset-opendal`](./packages/tape-dataset-opendal/README.md)         | `tape-dataset-opendal` | Exports standard Bub/Republic tapes to a backend-agnostic dataset layout through OpenDAL, with CEL filtering and staged share-review support.         |
